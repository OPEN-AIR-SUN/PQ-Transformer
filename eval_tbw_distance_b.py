    ...
    for batch_idx, batch_data_label in tqdm(enumerate(test_loader)):
        ...
        for prefix in prefixes:
            ...
            for b in range(end_points['point_clouds'].shape[0]):
                ...
                # np.save(f'statistics_tbw/distance-{end_points["scan_idx"][b].item()}.npy', distance_wall)
                plt.cla()
                plt.hist(distance_wall, bins=100, color='g', alpha=0.5)
                os.makedirs('statistics_tbw_distance_b', exist_ok=True)
                plt.savefig(f'statistics_tbw_distance_b/distribution-{end_points["scan_idx"][b].item()}.png')

                # distance = np.abs(distance)
                # distance_wall = distance[mask.cpu().numpy()]
                # d2c = lambda d: ((128 + min(1, d) * 127)/255, (255-min(1, d) * 127)/255, 1.)
                # with open(f'statistics_tbw/distance-{end_points["scan_idx"][b].item()}.txt', 'w') as f:
                ...
                # np.save(f'statistics_tbw/distance-abs-{end_points["scan_idx"][b].item()}.npy', distance_wall)
                # plt.cla()
                # plt.hist(distance_wall, bins=100, color='g', alpha=0.5)
