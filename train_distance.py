...
from models.loss_helper_pq import get_loss_distance
...
def parse_option():
    ...
    # Dataset Splitting
    parser.add_argument('--start_proportion', default=0.1, type=float, help='Start proportion of the dataset')
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        # Compute loss
        # Data labels except Semantic Labels are not acquirable here
        end_points['point_clouds'] = batch_data_label['point_clouds']
        end_points['semantic_labels'] = batch_data_label['semantic_labels']
        # for key in batch_data_label:
