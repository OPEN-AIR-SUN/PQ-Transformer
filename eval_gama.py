    ...
    for batch_idx, batch_data_label in tqdm(enumerate(test_loader)):
        ...
        for prefix in prefixes:
            ...
            for b in range(end_points['point_clouds'].shape[0]):
                ...
                mask_label = fit_gamma(distance_wall)
                threed_vertex = []
                for idx, each in enumerate(mask):
                    if each:
                        threed_vertex.append(pc_scene[idx])
                with open(f'statistics_detach_zcy/detached_point_txt/distance-{end_points["scan_idx"][b].item()}.txt', 'w') as f:
                        for idx, each in enumerate(mask_label):
                            if each:
                ...
                np.save(f'statistics_detach_zcy/distance-{end_points["scan_idx"][b].item()}.npy', distance_wall)
                plt.cla()
                plt.hist(distance_wall, bins=100, color='g', alpha=0.5)
                plt.savefig(f'statistics_detach_zcy/distribution-{end_points["scan_idx"][b].item()}.png')

                distance = np.abs(distance)
                distance_wall = distance[mask.cpu().numpy()]
                fit_gamma(distance_wall)
                d2c = lambda d: ((128 + min(1, d) * 127)/255, (255-min(1, d) * 127)/255, 1.)
                with open(f'statistics_detach_zcy/distance-{end_points["scan_idx"][b].item()}.txt', 'w') as f:
                ...
                np.save(f'statistics_detach_zcy/distance-abs-{end_points["scan_idx"][b].item()}.npy', distance_wall)
                plt.cla()
                plt.hist(distance_wall, bins=100, color='g', alpha=0.5)
                plt.savefig(f'statistics_detach_zcy/distribution-{end_points["scan_idx"][b].item()}-abs.png')
